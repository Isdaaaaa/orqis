...
function getWebAppHtml(): string {
  return [
    ...
    "      <div class=\"controls\">",
    "        <div class=\"row\">",
    "          <label>Workspace ID <input id=\"workspace-id\" value=\"default-workspace\" /></label>",
    "          <label>Project ID <input id=\"project-id\" value=\"default-project\" /></label>",
    "          <label>Actor type",
    ...
    "          : message.actorType;",
    "",
    "        meta.textContent = (message.createdAt ?? '') + ' • ' + actorLabel + ' • project=' + message.projectId;",
    "",
    ...
    "        body: JSON.stringify({",
    "          projectId: projectInput.value.trim() || undefined,",
    ...
    "    });",
    "",
    "    void reloadTimeline().catch((error) => {",
    "      setStatus(error instanceof Error ? error.message : String(error), true);",
    "    });",
    "  </script>",
    "</body>",

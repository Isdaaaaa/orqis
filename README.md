...
- PM planning now recognizes workflow-command goal prefixes (`implement:`, `review:`, `integrate:` plus `implementation:`/`integration:` aliases), routes tasks to phase-appropriate specialist roles, and posts explicit workflow status updates in the PM timeline message.
...
- Authenticated task output submission is available at `POST /api/workspaces/:workspaceId/tasks/:taskId/output`, which now auto-claims claimable task executions (`todo` / `in_progress`) for the submitted run+owner, stores the submitted output in the workspace timeline, creates or resubmits the task approval record, and moves the task/run into `waiting_approval`.

...
- Authenticated task output submission is available at `POST /api/workspaces/:workspaceId/tasks/:taskId/output`, which stores the submitted output in the workspace timeline, creates or resubmits the task approval record, and moves the task/run into `waiting_approval`.
